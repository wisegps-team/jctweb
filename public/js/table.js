...
_dataTable.prototype.fnServerData = function(query, afterSend, beforeSend, lookup){
    ...
    return function retrieveData( sSource, aoData, fnCallback ) {
        ...
        var url = '';
        if(_lookup){
            url = wistorm_api._lookupUrl(_this.tableName, _lookup, _query, _this.sFields, _this.sort, _this.sort, page_no, page_count, auth_code, true);
        }else{
            url = wistorm_api._listUrl(_this.tableName, _query, _this.sFields, _this.sort, _this.sort, 0, 0, page_no, page_count, auth_code, true);
        }
        $.ajax( {
            "type": "GET",
